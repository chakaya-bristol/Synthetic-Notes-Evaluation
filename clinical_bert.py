...
def run(name, csv_path):
    ...
    args = TrainingArguments(
        output_dir=str(OUT / f"clinicalbert_freegen_{name}"),
        num_train_epochs=3,
    ...
    save_dir = str(OUT / f"clinicalbert_freegen_{name}")
    ...
    }).to_csv(OUT / f"clinicalbert_preds_freegen_{name}.csv", index=False)
...
pd.DataFrame(results).to_csv(OUT / "clinicalbert_freegen_summary.csv", index=False)
print("\nSummary:")
